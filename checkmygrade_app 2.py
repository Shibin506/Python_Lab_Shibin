...
# Note: In production, you’d store this in an env variable or config file
# Ideally, load key from secure source. For now, using static dev key.
key = b'development-secret-key-must-be-32-bytes123'
cipher_suite = Fernet(key)

# Check if CSV file is missing; if so, write default headers
def ensure_csv_initialized(file_path, headers):
...
ensure_csv_initialized(STUDENT_FILE, ['Email_address', 'First_name', 'Last_name', 'Course.id', 'grades', 'Marks'])
ensure_csv_initialized(COURSE_FILE, ['Course_id', 'Course_name', 'Description'])
ensure_csv_initialized(PROFESSOR_FILE, ['Professor_id', 'Professor_Name', 'Rank', 'Course.id'])
ensure_csv_initialized(GRADES_FILE, ['Grade_id', 'Grade', 'Marks_range'])
ensure_csv_initialized(LOGIN_FILE, ['User_id', 'Password', 'Role'])
